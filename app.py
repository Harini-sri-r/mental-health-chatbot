            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a supportive AI wellness companion. Answer empathetically, "
                        "safely, and clearly without diagnosing or replacing professional care."
                    )
                },
...
def detect_topic(text):
    text = text.lower()
    if any(w in text for w in ["exam", "test", "grade", "study", "homework", "school", "college"]):
        return "exam"
    if any(w in text for w in ["relationship", "girlfriend", "boyfriend", "partner", "breakup"]):
        return "relationship"
    if any(w in text for w in ["family", "mom", "dad", "parents", "brother", "sister"]):
        return "family"
    if any(w in text for w in ["job", "work", "boss", "interview", "career"]):
        return "career"
    return "general"
...
def save_chat(sender, message, emotion="neutral", topic="general"):
    ...
    try:
        c.execute(
            "INSERT INTO chat (session_id, sender, message, emotion, topic) VALUES (?, ?, ?, ?, ?)",
    ...
    try:
        c.execute(
            """SELECT topic FROM chat
               WHERE session_id=? AND sender='user' AND topic != 'general'
...
# AI LLM RESPONSE
# =========================
def generate_ai_response(user_input, emotion, topic, user_name="friend"):
    state = session.get("therapy_state")
    lowered_input = user_input.lower()
    if "help me" in lowered_input or "guide me" in lowered_input or "therapy mode" in lowered_input:
    ...
    if state == "step_1":
        ...
        return "I see. How did that specific situation make you feel physically and emotionally?"
    if state == "step_2":
        session["therapy_state"] = "step_3"
        return "That is valid to feel. Looking at the situation, what is one small thing you can control right now?"
    if state == "step_3":
        session["therapy_state"] = None
        return "Excellent. Focus on that one controllable step. You can come back to this whenever you want to talk it through again."
    ...
    prompt = (
        f"You are Aura, a caring AI wellness companion. The user is feeling {emotion} and is talking about {topic}. "
        f"Respond with empathy, validation, and supportive guidance without sounding generic. "
        f"Keep the answer concise, safe, and human-like. Do not diagnose or replace professional care. "
    ...
    fallbacks = {
        "sad": [
            f"I'm sorry to hear that, {user_name}. Want to talk about it?",
            f"I'm here for you, {user_name}."
        ],
        "happy": [
            "That's great. Keep noticing what helped today.",
            "Love hearing that you are feeling lighter."
        ],
        "anxiety": [
            "Take a slow breath. Let's make the next step smaller.",
            "I understand. Anxiety can feel intense, but we can slow it down together."
        ],
        "angry": [
            "It's completely okay to be angry. Want to vent about it?",
            "Take your time. What triggered this?"
        ],
        "neutral": [
            "I'm listening.",
            "Tell me more."
...
def home():
    ...
    greeting = f"Hi, I'm Aura from MindEase AI. How are you feeling today, {session['user_name']}?"
...
def chat():
    try:
        ...
        if not user_input:
            return jsonify({"response": "Please type something so I can support you."})
        ...
        if crisis_detection(user_input, emotion):
            ...
            return jsonify({"response": response, "emotion": "sad"})
